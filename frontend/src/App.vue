...
import { Archive, CheckCircle2, FolderArchive, LayoutList, Menu, Settings, ShieldCheck, UsersRound, Wifi, X } from 'lucide-vue-next'
...
function closeMobileMenuOnEscape(event: KeyboardEvent) { if (event.key === 'Escape') mobileMenuOpen.value = false }
onMounted(() => { void refreshState(); window.addEventListener('keydown', closeMobileMenuOnEscape) })
onBeforeUnmount(() => window.removeEventListener('keydown', closeMobileMenuOnEscape))
...
<template>
  ...
  <div v-else class="app-shell">
    <header class="mobile-app-bar">
      ...
      <button class="quiet-button mobile-menu-toggle" :aria-expanded="mobileMenuOpen" aria-controls="mobile-more-menu" aria-label="打开更多菜单" @click="mobileMenuOpen = true"><Menu :size="21" /></button>
    </header>
    <aside class="sidebar">
      <RouterLink to="/tasks" class="brand"><span class="brand-mark"><Archive :size="20" /></span><span>Telegram 媒体归档</span></RouterLink>
    ...
    <nav class="mobile-bottom-nav" aria-label="主导航">
      <RouterLink to="/tasks" class="mobile-nav-link"><LayoutList :size="20" /><span>任务</span></RouterLink>
      <RouterLink to="/sources" class="mobile-nav-link"><UsersRound :size="20" /><span>来源</span></RouterLink>
      <RouterLink to="/archives" class="mobile-nav-link"><FolderArchive :size="20" /><span>归档</span></RouterLink>
    </nav>
    <div v-if="mobileMenuOpen" class="mobile-menu-backdrop" @click.self="mobileMenuOpen = false">
      <aside id="mobile-more-menu" class="mobile-more-menu" aria-label="更多菜单">
        <div class="mobile-more-head"><span>更多</span><button class="dialog-close" aria-label="关闭更多菜单" @click="mobileMenuOpen = false"><X :size="22" /></button></div>
        <RouterLink to="/settings" class="mobile-more-link" @click="mobileMenuOpen = false"><Settings :size="19" />设置</RouterLink>
        <section class="connection-card">
          <div class="connection-icon"><Wifi :size="19" /></div>
          <div><p><span :class="['status-dot', state?.accountConnected ? 'is-good' : state?.connectionStatus === 'invalid' ? 'is-warning' : 'is-muted']"></span>{{ state?.accountConnected ? 'Telegram 已连接' : state?.connectionStatus === 'invalid' ? '登录状态已失效' : '等待连接 Telegram' }}</p><small>{{ state?.accountConnected ? state.accountName : state?.connectionStatus === 'invalid' ? '请重新连接账号' : '完成连接后即可归档' }}</small></div>
        </section>
        <div v-if="state?.demoMode" class="demo-badge">演示模式 · 未连接真实 Telegram</div>
        <div class="local-note"><ShieldCheck :size="15" />数据仅存储在本机</div>
      </aside>
