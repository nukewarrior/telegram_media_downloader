...
<template>
  ...
  <section v-if="loading" class="loading-block">正在读取归档索引…</section><section v-else-if="!items.length" class="empty-state compact"><div class="empty-icon"><Search :size="26" /></div><h2>没有匹配的归档文件</h2><p>完成下载后的图片和视频会在这里生成缩略图。</p></section><section v-else class="archive-content"><div v-for="(monthItems, month) in groupedItems" :key="month" class="archive-month"><div class="section-title"><h2>{{ month }}</h2><span>{{ monthItems.length }} 个文件</span></div><div class="archive-grid"><button v-for="item in monthItems" :key="item.id" class="archive-card" @click="select(item)"><div :class="['media-preview', item.media_type.toLowerCase()]"><img v-if="item.thumbnail_url" :src="resource(item.thumbnail_url) ?? undefined" :alt="item.filename" /><template v-else><Image v-if="item.media_type === 'PHOTO'" :size="28" /><Video v-else-if="item.media_type === 'VIDEO'" :size="28" /><FileAudio v-else-if="item.media_type === 'AUDIO'" :size="28" /><FileText v-else :size="28" /><small v-if="previewStatus[item.thumbnail_status]">{{ previewStatus[item.thumbnail_status] }}</small></template><span v-if="item.media_type === 'VIDEO'" class="play-badge">▶</span></div><div><b>{{ item.filename }}</b><small>{{ item.chat_title }} · {{ new Date(item.message_date).toLocaleDateString('zh-CN') }}</small><span>{{ bytes(item.size_bytes) }}</span></div></button></div></div></section>
