...
<template>
  ...
  <section v-if="settings" class="settings-stack">
    ...
    <section class="settings-panel"><div class="settings-heading"><div class="settings-icon"><Clock3 :size="20" /></div><div><h2>归档时区</h2><p>新建任务会固定使用此 IANA 时区决定群组目录下的年月。</p></div></div><form class="setting-row setting-form-row" @submit.prevent="saveTimezone"><label>搜索时区<div class="input-with-icon"><Search :size="17" /><input v-model="timezoneSearch" placeholder="例如：Asia/Shanghai" autocomplete="off" /></div></label><label>当前时区<select v-model="archiveTimezone" required><option v-for="name in visibleTimezones" :key="name" :value="name">{{ name }}</option></select></label><div><button class="primary-button" type="submit">保存</button><small>不会移动已有文件，也不会改变已创建任务。</small></div></form><p v-if="timezoneSaved" class="success-note"><CheckCircle2 :size="17" />归档时区已保存</p></section>
    <section v-if="error" class="lan-warning"><ShieldAlert :size="21" /><div><b>设置未保存</b><p>{{ error }}</p></div></section>
    <section class="settings-panel"><div class="settings-heading"><div class="settings-icon"><Database :size="20" /></div><div><h2>并发下载</h2><p>全应用共享上限；遇到 Telegram 限流或连续网络错误会自动降载并在稳定后恢复。</p></div></div><form class="setting-row setting-form-row" @submit.prevent="saveConcurrency"><label>同时传输文件数<input v-model.number="concurrency" type="number" min="1" max="5" required /></label><div><button class="primary-button" type="submit">保存</button><small>当前有效并发 {{ settings.download.effectiveConcurrency }} / {{ settings.download.maxConcurrency }} · 活跃 {{ settings.download.activeDownloads }}</small></div></form><p v-if="concurrencySaved" class="success-note"><CheckCircle2 :size="17" />并发设置已保存</p></section>
    <section class="settings-panel"><div class="settings-heading"><div class="settings-icon"><Database :size="20" /></div><div><h2>来源浏览缓存</h2><p>已浏览媒体的索引会长期保留；超出上限时自动淘汰最久未访问的图片和视频缩略图。</p></div></div><form class="setting-row setting-form-row" @submit.prevent="saveSourceCache"><label>缩略图缓存上限（MiB）<input v-model.number="sourceCacheMiB" type="number" min="256" max="20480" required /></label><div><button class="primary-button" type="submit">保存</button><small>默认 2048 MiB；不会下载完整来源文件。</small></div></form><p v-if="sourceCacheSaved" class="success-note"><CheckCircle2 :size="17" />来源缓存上限已保存</p></section>
