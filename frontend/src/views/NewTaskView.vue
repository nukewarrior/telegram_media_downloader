...
import { ArrowLeft, ArrowRight, CalendarDays, Check, FileSearch, FolderKanban, HardDrive, Image, LoaderCircle, ScanSearch, Video } from 'lucide-vue-next'
...
const payload = () => ({ chat_id: selectedChat.value!.id, chat_title: selectedChat.value!.title, chat_handle: selectedChat.value!.handle, filters: { media_types: mediaTypes.value, date_start: dateStart.value || null, date_end: dateEnd.value || null, min_size_mb: minSize.value, max_size_mb: maxSize.value } })
...
onMounted(async () => { try { chats.value = await api.chats() } catch (reason) { error.value = reason instanceof Error ? reason.message : '无法读取聊天列表' } })
</script>
...
<template>
  ...
  <section v-if="step === 1" class="wizard-surface"><div class="wizard-copy"><span class="eyebrow">步骤 1</span><h2>选择一个来源聊天</h2><p>v1 中一个归档任务只处理一个频道或群聊，进度与来源更清晰。</p></div><label class="search-input"><FileSearch :size="18" /><input v-model="search" placeholder="搜索聊天名称" autofocus /></label><div class="chat-picker"><button v-for="chat in filteredChats" :key="chat.id" :class="['chat-choice', { selected: selectedChat?.id === chat.id }]" @click="selectedChat = chat"><span class="chat-avatar">{{ chat.title.slice(0, 1) }}</span><span><b>{{ chat.title }}</b><small>{{ chat.handle || (chat.type === 'GROUP' ? '群聊' : '频道') }}</small></span><Check v-if="selectedChat?.id === chat.id" :size="20" /></button></div><footer class="wizard-footer"><span>{{ selectedChat ? `已选择：${selectedChat.title}` : '请选择一个聊天' }}</span><button class="primary-button" :disabled="!selectedChat" @click="step = 2">继续<ArrowRight :size="18" /></button></footer></section>
  <section v-else-if="step === 2" class="wizard-surface filter-surface"><div class="wizard-copy"><span class="eyebrow">步骤 2</span><h2>设定归档范围</h2><p>{{ selectedChat?.title }} · {{ selectedChat?.handle }}</p></div><div class="filter-grid"><fieldset><legend>媒体类型</legend><div class="media-toggles"><button :class="{ selected: mediaTypes.includes('PHOTO') }" @click="toggle('PHOTO')"><Image :size="18" />图片</button><button :class="{ selected: mediaTypes.includes('VIDEO') }" @click="toggle('VIDEO')"><Video :size="18" />视频</button><button :class="{ selected: mediaTypes.includes('DOCUMENT') }" @click="toggle('DOCUMENT')"><FolderKanban :size="18" />文件</button></div></fieldset><fieldset><legend>时间范围</legend><div class="field-row"><label>开始日期<input v-model="dateStart" type="date" /></label><label>结束日期<input v-model="dateEnd" type="date" /></label></div></fieldset><fieldset><legend>文件大小（MB）</legend><div class="field-row"><label>最小<input v-model.number="minSize" type="number" min="0" /></label><label>最大（0 为不限）<input v-model.number="maxSize" type="number" min="0" /></label></div></fieldset></div><p v-if="error" class="form-error">{{ error }}</p><footer class="wizard-footer"><button class="quiet-button" @click="step = 1"><ArrowLeft :size="17" />上一步</button><button class="primary-button" :disabled="scanning || !mediaTypes.length" @click="scan"><LoaderCircle v-if="scanning" class="spin" :size="18" /><ScanSearch v-else :size="18" />{{ scanning ? '正在扫描…' : '扫描匹配内容' }}</button></footer></section>
  <section v-else class="wizard-surface confirmation-surface"><div class="wizard-copy"><span class="eyebrow">步骤 3</span><h2>确认归档内容</h2><p>扫描不会下载文件。确认后任务将进入队列。</p></div><div class="scan-summary"><div><span>匹配媒体</span><strong>{{ scanResult?.totalCount.toLocaleString() }}</strong><small>个文件</small></div><div><span>预计总大小</span><strong>{{ formatBytes(scanResult?.totalBytes ?? 0) }}</strong><small>原始媒体</small></div><div><span>可复用重复项</span><strong>{{ scanResult?.duplicateCount }}</strong><small>不重复落盘</small></div></div><div class="scan-note"><HardDrive :size="20" /><span>文件会归档至服务器配置的数据卷中；可在设置页查看规范根目录。</span></div><p v-if="error" class="form-error">{{ error }}</p><footer class="wizard-footer"><button class="quiet-button" @click="step = 2"><ArrowLeft :size="17" />返回修改</button><button class="primary-button" @click="create">创建并开始归档<ArrowRight :size="18" /></button></footer></section>
</template>
