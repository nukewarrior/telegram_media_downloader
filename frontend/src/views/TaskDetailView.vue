...
import { ArrowLeft, CirclePause, Clock3, Download, FileAudio, FileText, FileWarning, Gauge, Image, Play, RefreshCw, Video, X, XCircle } from 'lucide-vue-next'
import { api, apiResourceUrl, type Task, type TaskMedia, type TaskMediaPage } from '../api'
...
const statusLabel: Record<string, string> = { SCANNING: '正在扫描', DOWNLOADING: '正在下载', RETRYING: '等待自动重试', WAITING_RATE_LIMIT: 'Telegram 限流等待', PAUSED: '已暂停', COMPLETED: '已完成', FAILED: '失败', PARTIAL_FAILED: '部分失败', QUEUED: '等待中', CANCELLED: '已取消', PENDING: '等待下载', RETRY_WAIT: '等待重试' }
...
function markThumbnailFailed(item: TaskMedia) {
  failedThumbnailIds.value = new Set(failedThumbnailIds.value).add(item.id)
}
function openMediaPreview(item: TaskMedia) {
...
function handleKeydown(event: KeyboardEvent) {
  if (event.key === 'Escape' && selectedMedia.value) closeMediaPreview()
}
const eta = computed(() => {
...
function updateMedia(item: TaskMedia) {
  ...
  // Page 1 is current-first. Insert a newly active or failed item immediately
  // so the user never has to wait for a page reload to see what needs attention.
  if (mediaPage.value.page === 1 && (item.status === 'DOWNLOADING' || item.status === 'FAILED')) {
    mediaPage.value.items.unshift(item)
    mediaPage.value.items = mediaPage.value.items.slice(0, mediaPage.value.pageSize)
  }
}
...
async function action(kind: 'pause' | 'resume' | 'retry' | 'cancel') {
  ...
  try {
    task.value = await api.action(taskId, kind)
...
onMounted(async () => {
  ...
  await load()
  if (mediaPage.value) connectEvents(mediaPage.value.mediaRevision)
  retryClockTimer = window.setInterval(() => { retryClock.value = Date.now() }, 1000)
})
onBeforeUnmount(() => {
  window.removeEventListener('keydown', handleKeydown)
  source?.close()
  if (retryClockTimer !== null) window.clearInterval(retryClockTimer)
})
</script>
