...
import { ArrowLeft, CirclePause, Clock3, FileAudio, FileText, FileWarning, Gauge, Image, Play, RefreshCw, Video, XCircle } from 'lucide-vue-next'
...
const viewerItem = computed<MediaViewerItem | null>(() => {
  ...
  return {
    ...
    chatTitle: task.value?.chat_title,
    destinationName: task.value?.destination?.name ?? '未指定目的地',
    sourceLabel: `${mediaTypeLabel[item.media_type] || item.media_type} · 任务归档`,
    contentUrl: preview ? apiResourceUrl(preview.content_url) : null,
    downloadUrl: preview?.download_url ? apiResourceUrl(preview.download_url) : null,
  }
})
...
<template>
  ...
  <MediaViewer :open="Boolean(selectedMedia)" :item="viewerItem" :error="viewerError" :has-previous="selectedIndex > 0" :has-next="selectedIndex >= 0 && selectedIndex < viewerItems.length - 1" @close="closeMediaPreview" @previous="navigateMedia(-1)" @next="navigateMedia(1)" />
</template>
