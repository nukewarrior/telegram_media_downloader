...
export type Task = { id: number; chat_id: string; chat_title: string; chat_handle: string | null; status: string; total_count: number; completed_count: number; failed_count: number; total_bytes: number; downloaded_bytes: number; current_file: string | null; speed_bytes_per_second: number; media_revision: number; error_message: string | null; download_wait_until: string | null; destination_id: number | null; destination: Destination | null; filters: Record<string, unknown>; created_at: string; updated_at: string; activeMedia?: TaskMedia[]; downloadRuntime?: DownloadRuntime }
