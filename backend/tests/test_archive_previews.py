...
class ArchivePreviewTests(unittest.TestCase):
    ...
    def test_photo_thumbnail_and_controlled_endpoints(self) -> None:
        ...
        with TestClient(main.app) as client:
            ...
            self.assertEqual(detail.json()["thumbnail_url"], f"/api/archives/media/{item_id}/thumbnail?v=test-PHOTO-example.png")
            self.assertIn("?v=", detail.json()["content_url"])
            self.assertIn("?v=", detail.json()["download_url"])
